    ...
    <div class="grid gap-3" style="grid-template-columns: repeat(auto-fill, minmax(min(200px, 100%), 1fr));">
        @forelse ($posts as $post)
            <a href="{{ route('posts.show', $post) }}" class="block rounded overflow-hidden cursor-default">
                <div class="relative flex items-center justify-center rounded overflow-hidden" data-thumb-container
                    ...
                    <div class="absolute inset-0" data-thumb-fit>
                        ...
                        @else
                            <img data-thumb-media
                                src="{{ \Illuminate\Support\Facades\Storage::disk('public')->url($post->thumbnail_path) }}"
                                alt="post {{ $post->id }}" loading="lazy" class="block w-full h-full object-contain cursor-pointer">
                        @endif
                    </div>
                </div>
            </a>
